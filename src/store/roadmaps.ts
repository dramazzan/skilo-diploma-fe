...
import { mockRoadmaps, mockRoadmapTrees, mockUserCollection, type RoadmapLevel, type RoadmapNode } from "@/mocks/mockRoadmaps"
import { api, type RoadmapProgressItem } from "@/services/api"
...
export const useRoadmapsStore = defineStore("roadmaps", () => {
  ...
  const loadUserRoadmapCollection = async (userId: number | null) => {
    ...
    const roadmapIds = await api.getUserRoadmapCollection(userId)
  ...
  const loadRoadmapProgress = async (userId: number | null) => {
    ...
    const items = await api.getRoadmapProgress(userId)
  ...
  const addRoadmapWithLevel = async (roadmapId: string, level: RoadmapLevel, userId: number | null) => {
    ...
    const savedCollection = await api.updateUserRoadmapCollection(userId, nextCollection)
  ...
  const removeRoadmapFromCollection = async (roadmapId: string, userId: number | null) => {
    ...
    try {
      const savedCollection = await api.removeUserRoadmapFromCollection(userId, roadmapId)
      setUserRoadmapCollection(savedCollection)
      persist()
    } catch (error) {
