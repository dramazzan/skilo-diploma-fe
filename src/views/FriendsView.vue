...
import { mockRoadmapAssessments } from "@/mocks/mockRoadmaps"
...
const loadChallenges = async (userId: number | null) => {
  const [challengesData, notificationsData] = await Promise.all([
    api.getFriendChallenges(userId),
    api.getFriendChallengeNotifications(userId)
  ])
...
const markChallengeNotificationRead = async (challengeId: string) => {
  try {
    ...
    const userId = authStore.user?.id ?? null
    await api.markFriendChallengeNotificationRead(userId, challengeId)
    await loadChallenges(userId)
  } finally {
...
const submitChallengeQuiz = async () => {
  ...
  try {
    ...
    const userId = authStore.user?.id ?? null
    await api.createFriendChallenge(userId, {
