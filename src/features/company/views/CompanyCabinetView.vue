...
const loadData = async () => {
  try {
    ...
    const [vacancyList, candidateList] = await Promise.all([
      companyApi.getCompanyVacancies(),
      companyApi.getCompanyCandidates()
    ])
...
const createVacancy = async () => {
  ...
  try {
    ...
    await companyApi.createCompanyVacancy(toVacancyPayload(publishForm))
...
const saveVacancy = async (vacancyId: string) => {
  ...
  try {
    actionLoading.value = true
    await companyApi.updateCompanyVacancy(vacancyId, toVacancyPayload(editForm))
...
const addTask = async (vacancyId: string) => {
  ...
  try {
    actionLoading.value = true
    await companyApi.createCompanyVacancyTask(vacancyId, {
...
const saveTask = async () => {
  ...
  try {
    actionLoading.value = true
    await companyApi.updateCompanyVacancyTask(taskEdit.vacancyId, taskEdit.taskId, {
...
const sendInvite = async (candidateId: string) => {
  ...
  try {
    actionLoading.value = true
    const result = await companyApi.sendInterviewInvite(candidateId, {
